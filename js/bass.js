(function () {
    ...
    function buildBoard() {
        ...
        tuning.forEach(openNote => {
            ...
            // and clickable on its own to toggle the manual "note finder"
            // highlight, same as every other fret. Mirrors the Guitar
            // Scale Explorer's gtr-open-note-badge behavior.
    ...
    function repaint() {
        ...
        document.querySelectorAll('.open-badge').forEach(badge => {
            ...
            badge.classList.toggle('bass-root-match', !!isRootMatch);
            badge.classList.toggle('scale-root-ring', !!isScaleRoot);

            badge.textContent = (isScaleMatch || isRootMatch) ? 'O' : '•';
        });
