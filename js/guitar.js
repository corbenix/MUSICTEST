(function () {
    ...
    // Electric samples, swapped in whenever the Amp pedal is engaged.
    const GUITAR_ELECTRIC_SAMPLES = [
        ['A', 2], ['A', 3], ['A', 4],
        ['C', 3], ['C', 4], ['C', 5],
        ['Ds', 3], ['Ds', 4], ['Ds', 5],
        ['Fs', 3], ['Fs', 4], ['Fs', 5],
        ...
        return { url: `audio/guitar-electric/${file}${oct}.mp3`, abs: oct * 12 + MT.noteIndex(sharpNote) };
    });

    function ampIsOn() {
        return !!(window.GuitarEffects && window.GuitarEffects.state.amp && window.GuitarEffects.state.amp.on);
    }

    function nearestGuitarSample(targetAbs) {
        const samples = ampIsOn() ? GUITAR_ELECTRIC_SAMPLES : GUITAR_SAMPLES;
        let best = samples[0];
    ...
    function renderCaged() {
        ...
        cagedAutoLabel.style.color = shapesShown.length > 0 ? '#e6afff' : 'var(--text3)';
    ...
    const VOICING_SHAPE_COLOR = { E: '#e6afff', A: '#e6afff', D: '#e6afff', Open: '#e6afff' };
    ...
    function voicingChipSvg(dots) {
        ...
        perString.forEach((f, s) => {
            ...
            svg += `<circle cx="${x}" cy="${y}" r="4" fill="#c34dff" stroke="#e6afff" stroke-width="1"/>`;
        });
        svg += '</svg>';
        return svg;
    }
    ...
    function renderVoicingChips() {
        ...
        chordLibVoicings.forEach((v, i) => {
            ...
            const color = VOICING_SHAPE_COLOR[v.shape] || '#c34dff';
    ...
    // ── Capo Explorer ───────────────────────────────────────────────────
    // Two-step flow adapted from the reference app's dedicated Capo
    // Explorer panel: pick a capo fret (pill row, 0 = No Capo), then pick
    // which CAGED open shape you're fretting (colored chips), and a
    // sounding-chord card shows what actually rings out — mirroring the
    ...
    // Slightly brighter variant of each shape color, used for the active
    // card's accent border/glow — ported from the reference app's hpVivid.
    const CAPO_SHAPE_VIVID = { C: '#d98663', A: '#6596d2', G: '#3dc26f', E: '#cf4e4e', D: '#cfac30' };
    // The 10 open-shape qualities every CAGED letter is fretted in, in
    // display order, with their label captions and target-chord text
    // colors — ported from the reference app's gcQualityOrder / _c / ma.
    const CAPO_QUALITY_ORDER = ['', 'm', '7', 'maj7', 'm7', 'sus4', '5', 'sus2', 'dim', 'aug'];
    const CAPO_QUALITY_LABEL = {
    ...
    // ── Capo slider (stepper) ────────────────────────────────────────
    // Draggable/clickable track + +/- buttons, ported from the reference
    // app's .gc-capo-mobile-stepper. Same 0-9 fret range as the pill row
    // it replaces; 0 = "Open" (no capo).
    const capoSliderWord = document.getElementById('capo-slider-word');
    const capoSliderNum = document.getElementById('capo-slider-num');
    const capoSliderFill = document.getElementById('capo-slider-fill');
    const capoSliderDot = document.getElementById('capo-slider-dot');
    const capoSliderTrack = document.getElementById('capo-slider-track');
    ...
    function renderCapoSlider() {
        const fret = capoSelectedFret;
        if (fret === 0) {
            capoSliderWord.textContent = 'Open';
            capoSliderNum.textContent = '';
        } else {
            capoSliderWord.textContent = 'Fret';
            capoSliderNum.textContent = String(fret);
        }
        const pct = (fret / CAPO_MAX_FRET) * 100;
        capoSliderFill.style.width = pct + '%';
        capoSliderDot.style.left = pct + '%';
        capoSliderTrack.setAttribute('aria-valuenow', String(fret));
        capoSliderMinus.disabled = fret <= 0;
        capoSliderPlus.disabled = fret >= CAPO_MAX_FRET;
    }

    capoSliderMinus.addEventListener('click', () => setCapoFret(capoSelectedFret - 1));
    capoSliderPlus.addEventListener('click', () => setCapoFret(capoSelectedFret + 1));

    function fretFromPointerEvent(e) {
        const rect = capoSliderTrack.getBoundingClientRect();
        const clientX = (e.touches && e.touches[0]) ? e.touches[0].clientX : e.clientX;
        const ratio = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
        return Math.round(ratio * CAPO_MAX_FRET);
    }
    let capoDragging = false;
    capoSliderTrack.addEventListener('pointerdown', (e) => {
        capoDragging = true;
        capoSliderTrack.setPointerCapture && capoSliderTrack.setPointerCapture(e.pointerId);
        setCapoFret(fretFromPointerEvent(e));
    });
    capoSliderTrack.addEventListener('pointermove', (e) => {
        if (!capoDragging) return;
        setCapoFret(fretFromPointerEvent(e));
    });
    capoSliderTrack.addEventListener('pointerup', () => { capoDragging = false; });
    capoSliderTrack.addEventListener('pointercancel', () => { capoDragging = false; });
    capoSliderTrack.addEventListener('keydown', (e) => {
        if (e.key === 'ArrowRight' || e.key === 'ArrowUp') { setCapoFret(capoSelectedFret + 1); e.preventDefault(); }
        else if (e.key === 'ArrowLeft' || e.key === 'ArrowDown') { setCapoFret(capoSelectedFret - 1); e.preventDefault(); }
    });

    function capoSyncChipStates() {
        capoShapeChips.querySelectorAll('.root-pill').forEach(c => {
            const active = !capoShowAllShapes && c.dataset.shape === capoSelectedShape;
    ...
    CAPO_SHAPES.forEach(shape => {
        const chip = document.createElement('button');
        chip.type = 'button';
        chip.className = `root-pill capo-shape-chip caged-${shape}`;
        ...
        capoShapeChips.appendChild(chip);
    });

    capoAllShapesBtn.addEventListener('click', () => {
        capoShowAllShapes = true;
    ...
    // ── Chord-card grid ──────────────────────────────────────────────
    // One .capo-group per CAGED shape (colored dot + "X SHAPE" + chord
    // count), each holding all 10 quality cards for that shape at the
    // current capo fret — ported from the reference app's
    // gc-capo-table-grid / .gc-capo-group / .gc-capo-card. Clicking a
    // card selects that exact shape+quality (drives the sounding-chord
    // card and fretboard below).
    function renderCapoCards() {
        capoCardGrid.innerHTML = '';
        if (capoSelectedFret === 0) return;

        if (!capoSelectedShape && !capoShowAllShapes) {
            const placeholder = document.createElement('div');
            placeholder.className = 'capo-card-grid-placeholder';
            placeholder.textContent = 'Choose a CAGED shape above — or "All Shapes" — to view its chords.';
            capoCardGrid.appendChild(placeholder);
            return;
        }

        const shapesToShow = capoShowAllShapes ? CAPO_SHAPES : [capoSelectedShape];
        shapesToShow.forEach(shape => {
            const color = CAPO_SHAPE_COLORS[shape];
            const soundingRoot = capoNoteName(MT.noteIndex(shape) + capoSelectedFret);

            const group = document.createElement('div');
            group.className = 'capo-group';
            const header = document.createElement('div');
            header.className = 'capo-group-header';
            header.style.color = color;
            header.innerHTML = `<span class="capo-group-dot" style="background:${color};color:${color};"></span>${shape} SHAPE<span class="capo-group-count">${CAPO_QUALITY_ORDER.length} chords</span>`;
            group.appendChild(header);

            const grid = document.createElement('div');
            grid.className = 'capo-group-grid';
            CAPO_QUALITY_ORDER.forEach(q => {
                const key = shape + q;
                const targetName = soundingRoot + q;
                const qColor = CAPO_QUALITY_COLOR[q] || 'var(--text)';
                const isActive = key === capoActiveKey;

                const card = document.createElement('div');
                card.className = 'capo-card' + (isActive ? ' active' : '');
                card.style.borderLeftColor = color;
                if (isActive) card.style.setProperty('--capo-card-accent', CAPO_SHAPE_VIVID[shape]);
                card.innerHTML = `
                    <div class="capo-card-top">
                        <span>Shape played</span>
                        <span class="capo-card-badge" style="background:${color}22;color:${color};border:1px solid ${color}66;">${shape}</span>
                    </div>
                    <div class="capo-card-body">
                        <div class="capo-card-from"><span class="capo-card-shape">${shape}${q}</span></div>
                        <span class="capo-card-arrow">→</span>
                        <div class="capo-card-to">
                            <span class="capo-card-target" style="color:${qColor};">${targetName}</span>
                            <span class="capo-card-quality">${CAPO_QUALITY_LABEL[q]}</span>
                        </div>
                    </div>
                `;
            ...
            capoCardGrid.appendChild(group);
        });
    }
    ...
    function renderCapo() {
        // Shape row only makes sense once a capo position is picked —
        // hide it (and the card grid) at "Open", same as the
        // reference's shapeRow.style.display = fret > 0 ? '' : 'none'.
        document.getElementById('capo-shape-row').style.display = capoSelectedFret > 0 ? '' : 'none';

        // With no capo, or no specific quality card picked yet, hide the
        // sounding-chord result card.
        if (capoSelectedFret === 0 || !capoActiveKey) {
            capoSoundingCard.hidden = true;
        } else {
            const shape = capoActiveKey[0];
            const quality = capoActiveKey.slice(1);
            const soundingRoot = capoNoteName(MT.noteIndex(shape) + capoSelectedFret);
